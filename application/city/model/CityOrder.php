...
class CityOrder extends Model
{
    ...
    public function WeChatPayCode($order_number){
        ...
        if($data){
            ...
            $input->SetNotify_url(config("domain2.url")."/city_meal_notify"); //回调地址
    ...
    public function AlipayCode($order_number)
    {
        ...
        if($data){
            ...
            $arr_data = array(
                "return_url" => trim(config("domain2.url")),
                "notify_url" => trim(config("domain2.url")."/city_meal_notify_alipay.html"),
