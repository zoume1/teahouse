...
{   
    public function doPageBdAppbase(){
        ...
        $code = input('code');
        $app = Db::table('ims_sudu8_page_bd_applet') ->where('uniacid', $uniacid) ->find();
        $appkey = $app['appkey'];
...
//        $diy_bg_music = pdo_getcolumn("sudu8_page_base", array("uniacid" => $uniacid), "diy_bg_music");
    ...
    public function doPageDiyForms()
    {
        ...
        $forms=Db::table('ims_sudu8_page_formcon')->where('fid', $data['fid'])->where('creattime',$data['creattime'])->where('uniacid',$uniacid)->find();
...
// 付费视频操作
    ...
    public function getComment()
    {
        ...
        if ($comment) {
            foreach ($comment as $k => &$v) {
                ...
                //查询该评论下的点赞
                $zans = Db::table('ims_sudu8_page_art_comment_zan') ->where('uniacid', $uniacid) ->where('comid', $v['id'])->field('zan, openid') ->select();
                if($zans){
    ...
    public function commentFollow()
    {
        ...
        $zan_rec = Db::table('ims_sudu8_page_art_comment_zan') ->where('uniacid', $uniacid) ->where('comid', $id) ->where('openid', $openid) ->find();
        if($zan == 2){
            if(!$zan_rec){
                Db::table('ims_sudu8_page_art_comment_zan') ->insert(['uniacid'=>$uniacid, 'comid'=>$id, 'openid'=>$openid, 'zan'=>1, 'createtime'=>time()]);
            }else{
                Db::table('ims_sudu8_page_art_comment_zan') ->where('uniacid', $uniacid) ->where('comid', $id) ->where('openid', $openid) ->update(['zan'=> 1]);
            }
            ...
            if(!$zan_rec){
                Db::table('ims_sudu8_page_art_comment_zan') ->insert(['uniacid'=>$uniacid, 'comid'=>$id, 'openid'=>$openid, 'zan'=>2, 'createtime'=>time()]);
            }else{
                Db::table('ims_sudu8_page_art_comment_zan') ->where('uniacid', $uniacid) ->where('comid', $id) ->where('openid', $openid) ->update(['zan'=> 2]);
            }
            $follow = intval($follow['follow']) - 1;
            $data = array(
                'id' => $id,
...
//                exit();
    ...
    public function doPagegetcate()
    {
        $cid = input('cid');
        $uniacid = input('uniacid');
        $subcate = Db::table('ims_sudu8_page_shops_cate')->where('uniacid', $uniacid)->field('id,name')->select();
        $result['data'] = $subcate;
        return json_encode($result);
    }
...
//        if ($config['miniadT'] == "2") {
    ...
    public function doPageIndex_hot()
    {
        ...
        $list_y = Db::query("SELECT id,type,num,title,thumb,ctime,hits,`desc`,price,market_price,sale_num,buy_type,is_more FROM ims_sudu8_page_products WHERE type_y=1 and flag = 1 and is_sale=0 and uniacid = " . $uniacid . " and (type = 'showArt' or type = 'showPic') ORDER BY num DESC,id DESC LIMIT 0,{$max_pt}");
    ...
    public function doPagelistPic()
    {
        ...
        // 判断栏目等级 根据等级显示不同的信息
        $cateinfos = Db::table('ims_sudu8_page_cate')->where("uniacid", $uniacid)->where("id", $cid)->field("id,cid,type,pagenum")->find();
        ...
        if ($cateinfo['type'] == 'showArt' or $cateinfo['type'] == 'showPic' or $cateinfo['type'] == 'showPro') {
            //获取所有数量
            $cateinfo['num'] = Db::table('ims_sudu8_page_products')->where('uniacid', $uniacid)->where($slid, $cid)->field('id')->select();
            //获取栏目文章
            $cateinfo['list'] = Db::table('ims_sudu8_page_products')->where('uniacid', $uniacid)->where($slid, $cid)->field('id,type,num,title,thumb,ctime,edittime,etime,hits,`desc`,price,market_price,sale_num,is_more,buy_type,sale_tnum')->order('num desc,id desc')->limit(($pindex - 1) * $psize, $psize)->select();
            foreach ($cateinfo['list'] as &$row) {
                ...
                if ($row['is_more'] == 3) {
                    $pro_type_val = Db::table('ims_sudu8_page_duo_products_type_value') ->where('pid', $row['id']) ->select();
                    if($pro_type_val){
    ...
    public function doPageshowPro()
    {
        ...
        // $pro['userinfo'] = pdo_get("sudu8_page_user", array("uniacid"=>$uniacid, "openid"=>$openid));
        $pro['userinfo'] = Db::table('ims_sudu8_page_user') ->where('uniacid', $uniacid) ->where('openid', $openid) ->find();
    ...
    public function doPageOrderinfo()
    {
        ...
        if ($pro['is_more'] == "1" or $pro['is_more'] == 1) {
            ...
            if ($now > $orders['overtime'] && $orders['flag'] == 0) {   //订单失效
                ...
                // //处理优惠券
                // if($orders['coupon']){
                //     Db::table('ims_sudu8_page_coupon_user') ->where('uniacid', $uniacid) ->where('uid', $orders['uid']) ->where('id', $orders['coupon']) ->update(array('flag' => 0,"utime"=>0));
                // }
                // if($orders['dkscore']){
                //     Db::execute("UPDATE ims_sudu8_page_user set score = score + ".$orders['dkscore']." where uniacid = ".$uniacid." and id = ".$orders['uid']);
                //     $score_data = array(
                //         "uniacid" => $uniacid,
                ...
                //     Db::table('ims_sudu8_page_products') ->where('uniacid', $uniacid) ->where('id', $pro['id']) ->update(['sale_tnum' => $sale_tnum, 'more_type_num' => serialize($more_type_num), 'more_type'=> serialize($more_type)]);
                // }else{
                //     //更新选择座位状态
                //     Db::table('ims_sudu8_page_tableselect')->where('id', $orders['tsid']) ->update(['flag' => 2]);
                //     // $has_select = unserialize($orders['order_duo']);
                //     //减少销量
                //     $table_select = Db::table('ims_sudu8_page_tableselect') ->where('id', $orders['tsid']) ->find();
                //     $temp_select = explode(',', $table_select['select_str']);
                //     $count = count($temp_select);

                //     $sale_tnum = $product['sale_tnum'] - $count;
                //     Db::table('ims_sudu8_page_products') ->where('id', $pro['id']) ->update(['sale_tnum'=>$sale_tnum]);
                // }
    ...
    public function doPageMyorder(){
        ...
        foreach ($wforders as $key => &$res) {
             $now = time();
            if ($now > $res['overtime'] && $res['flag'] == 0){
                $adata = array(
                    "flag" => -1
                );
                ...
                $orders = Db::table('ims_sudu8_page_order')->where('id', $res['id']) ->find();
        ...
        // 自动收货
        $receiving_day = Db::table('ims_sudu8_page_duo_products_yunfei') ->where('uniacid', $uniacid) ->field('receiving') ->find();
        if(intval($receiving_day['receiving']) > 0){
            // $hxlist = pdo_fetchall("SELECT * FROM ".tablename('sudu8_page_order')." WHERE uniacid = :uniacid and flag = 4",array(':uniacid' => $uniacid));
            $hxlist = Db::table('ims_sudu8_page_order') ->where('uniacid', $uniacid) ->where('flag', 4) ->select();
            foreach ($hxlist as $kv => $vv) {
                if($vv['custime'] == null){
                    // pdo_query("UPDATE ".tablename("sudu8_page_order")." SET flag = 2 where id = ".$vv['id']." and uniacid = :uniacid and flag = 4", array(":uniacid"=>$uniacid));
                    Db::table('ims_sudu8_page_order') ->where('uniacid', $uniacid) ->where('flag', 4) ->where('id', $vv['id']) ->update(['flag' => 2]);
                }else{
                    $receiving = intval($receiving_day['receiving'])*24*3600;
        ...
        foreach ($OrdersList['list'] as  &$row){
            ...
            if($row['tsid'] > 0){
                // $tableselect = pdo_get("sudu8_page_tableselect", array("uniacid"=>$uniacid, "id"=>$row['tsid']));
                $tableselect = Db::table('ims_sudu8_page_tableselect') ->where('uniacid', $uniacid) ->where('id', $row['tsid'])->find();
                $row['select_str'] = $tableselect['select_str'];
                $row['appoint_date'] = $tableselect['appoint_date'];
            }

            if($row['emp_id']){
                $emp = Db::table('ims_sudu8_page_staff') ->where('id', $row['emp_id'])->field('realname, mobile') ->find();
                if($emp){
                    $row['emp_name'] = $emp['realname'];
                    $row['emp_mobile'] = $emp['mobile'];
                }
...
//                        $kcl += $res['kc'];
                    ...
                    $pro = Db::table('ims_sudu8_page_shops_goods')->where("uniacid", $uniacid)->where("id", $rec['cid'])->where("flag", 1)->find();
                    if($pro){
    ...
    public function doPageProductsearch()
    {
        ...
        $paimai = input('paimai');
        $flag = 1;
        $auction = Db::table('ims_sudu8_page_auction_goodslist')->where("uniacid", $uniacid)->where("stat", '<>', 0)->where("stat", '<>', -1)->where("name", 'like', $title)->field('id,name as title,img as thumb,starttime,endtime,stat')->order("id desc")->select();
            ...
            if($product){
                foreach ($product as &$row){
                    if($row['type'] == "showProMore"){
                         $a=Db::table("ims_sudu8_page_duo_products_type_value")->where("pid",$row['id'])->min("price");
                        $row['price']=$a;
                    }
                    if($row['is_more']==0){
...
//                        $row['sale_num'] = $row['sale_tnum'];
                        $b=Db::table("ims_sudu8_page_duo_products_type_value")->where("pid",$row['id'])->sum("salenum");
                        $c=Db::table("ims_sudu8_page_duo_products_type_value")->where("pid",$row['id'])->sum("vsalenum");
                        $row['sale_num']=$b+$c;
                    }
                    $row['thumb'] = remote($uniacid, $row['thumb'], 1);
            ...
            if(!empty($auction) && !empty($product)){
                $result = array_merge($auction,$product);   
            }else if(empty($auction) && !empty($product)){
              $result = $product;
            }else if(!empty($auction) && empty($product)){
              $result = $auction;
            }
...
//            "formid"=>$formid,
    ...
    public function dopageScoreinfo()
    {
        ...
        if ($id) {
            ...
            //查询是否已收藏
            $res = Db::table('ims_sudu8_page_collect')->where("uniacid", $uniacid)->where("uid", $uid)->where("type", "exchange")->where("cid", $id)->find();
            if($res){
                $products['sc'] = 1;
            }else{
                $products['sc'] = 0;
...
//            $pro_belong = pdo_fetch("select * from ".tablename("sudu8_page_pt_pro")." where id = :pid",array(':pid' => $vv['pvid']));
    ...
    public function dopagegetmzh()
    {
        ...
        $userinfo = Db::table('ims_sudu8_page_user')->where("uniacid", $uniacid)->where('openid', $openid)->find();
        // 申请提现中的钱
        $ord = Db::table('ims_sudu8_page_fx_tx')->where("uniacid", $uniacid)->where('openid', $openid)->where('flag', 1)->select();
        $wfmoney = 0;
    ...
    public function dopagesqcwfxs()
    {
        ...
        $openid = input("openid");
        $m=Db::table("ims_sudu8_page_user")->where("uniacid",$uniacid)->where('openid',$openid)->where('fxs',2)->find();
//        $fxsstop = pdo_fetchcolumn("SELECT fxsstop FROM ".tablename('sudu8_page_user')." WHERE uniacid = :uniacid and openid = :openid", array(":uniacid" => $uniacid, ":openid" => $openid));
        if($m['fxsstop'] == 2){
            $data=array(
                "openid" => $openid,
                "uniacid" => $uniacid,
        ...
        if($guiz['fxs_sz'] == "1"){
            ...
            $f=Db::table("ims_sudu8_page_user")->where("uniacid",$uniacid)->where('openid',$openid)->where('fxs',2)->find();
            if($f['fxsstop'] == 2){
                  Db::table("ims_sudu8_page_user")->where("uniacid",$uniacid)->where('openid',$openid)->where('fxs',2)->update(array("fxsstop"=>1));
            }
        }
        }
        $result['data'] = $flag;
...
//        var_dump($fxsinfo);exit();
    ...
    public function doPagegetmygwc()
    {
        ...
        $userinfo = Db::table('ims_sudu8_page_user')->where('openid', $openid)->where('uniacid', $uniacid)->find();
        $uid = $userinfo['id'];
        $mygwc = Db::table('ims_sudu8_page_duo_products_gwc')->alias("a")->join("ims_sudu8_page_products b", "a.pvid = b.id")->where('a.uid', $uid)->where('a.flag', 1)->where('b.id', 'gt', 0)->field("a.*,b.kuaidi")->select();
        $guiz = Db::table('ims_sudu8_page_fx_gz')->where('uniacid', $uniacid)->find();
//        $array1=array();
        foreach ($mygwc as $key => &$res) {
//              if(!in_array($res['kuaidi'],$array1)){
//                  array_push($array1,$res['kuaidi']);
//              }
...
//        $jf_gz = pdo_fetch("SELECT * FROM ".tablename('sudu8_page_rechargeconf')." WHERE uniacid = :uniacid" , array(':uniacid' => $uniacid));
        ...
        // 我的积分抵用
//        $userinfo = pdo_fetch("SELECT * FROM ".tablename('sudu8_page_user')." WHERE openid = :openid and uniacid = :uniacid" , array(':openid' => $openid ,':uniacid' => input('uniacid']));
       $userinfo=Db::table("ims_sudu8_page_user")->where("openid",$openid)->where("uniacid",$uniacid)->find();
...
//            var_dump($fxsid);exit;
    ...
    public function dopageduoorderlist()
    {
            ...
            if ($st < time()) {
                ...
                if($res['sid'] != 0){  
                    ...
                    foreach ($res['jsondata'] as $key => &$reb) {
                        $sgoods = Db::table('ims_sudu8_page_shops_goods') ->where('id', $reb['baseinfo']['id']) ->find();
                        $rsales = $sgoods['rsales'] - $reb['num'];
                        $storage = $sgoods['storage'] + $reb['num'];
                        Db::table('ims_sudu8_page_shops_goods') ->where('id', $reb['baseinfo']['id']) ->update(['rsales' => $rsales, 'storage' => $storage]);
                    }
                }
                Db::table('ims_sudu8_page_duo_products_order')->where('id', $res['id'])->update($adata);
                Db::table('ims_sudu8_page_fx_ls')->where('uniacid', $uniacid)->where('order_id', $res['order_id'])->update($adata);
        ...
        if ($flag == 0) {
            $orders = Db::table('ims_sudu8_page_duo_products_order')->where('uniacid', $uniacid)->where('openid', $openid)->where('flag', 0)->order('id desc')->limit($pagebegin, $pagesize)->select();
        }
        if ($flag == 1 && $type1 == 1) {
            $orders = Db::table('ims_sudu8_page_duo_products_order')->where('uniacid', $uniacid)->where('openid', $openid)->where('flag', 1)->where('nav', 1)->order('id desc')->limit($pagebegin, $pagesize)->select();
        }
        if ($flag == 1 && $type1 == 2) {
            $orders = Db::table('ims_sudu8_page_duo_products_order')->where('uniacid', $uniacid)->where('openid', $openid)->where('flag', 1)->where('nav', 2)->order('id desc')->limit($pagebegin, $pagesize)->select();
        }
        if ($flag == 2) {
            $orders = Db::table('ims_sudu8_page_duo_products_order')->where('uniacid', $uniacid)->where('openid', $openid)->where('flag', 2)->order('id desc')->limit($pagebegin, $pagesize)->select();
        }
        if ($flag == 3) {
            $orders = Db::table('ims_sudu8_page_duo_products_order')->where('uniacid', $uniacid)->where('openid', $openid)->where('flag', 3)->order('id desc')->limit($pagebegin, $pagesize)->select();
        }
        if ($flag == 4) {
            $orders = Db::table('ims_sudu8_page_duo_products_order')->where('uniacid', $uniacid)->where('openid', $openid)->where('flag', 4)->order('id desc')->limit($pagebegin, $pagesize)->select();
        }
        if ($flag == 5) {
            $orders = Db::table('ims_sudu8_page_duo_products_order')->where('uniacid', $uniacid)->where('openid', $openid)->where('flag', 5)->order('id desc')->limit($pagebegin, $pagesize)->select();
        }
        if ($flag == 7) {
            $orders = Db::table('ims_sudu8_page_duo_products_order')->where('uniacid', $uniacid)->where('openid', $openid)->where('flag',"in",[6,7,8,9])->order('id desc')->limit($pagebegin, $pagesize)->select();
        }
...
//    // 点击确认订单按钮确认收货（新版）
    public function dopagenewquerenxc(){
        ...
        if($order['sid'] != 0){
            //获取结算费率
            $jiesuan = Db::table('ims_sudu8_page_shops_set') ->where('uniacid', $uniacid) ->find();
            $jiesuan = $jiesuan['jiesuan'];
            $money = Db::table('ims_sudu8_page_shops_shop')->where("uniacid",$uniacid)->where("id",$order['sid'])->find()['tixian'];
            ...
            $result = Db::table('ims_sudu8_page_shops_shop')->where("uniacid",$uniacid)->where("id",$order['sid'])->update(array('tixian' => $money));
            exit;
        }
        if ($fxsorder) {
            $this->dopagegivemoney($uniacid, $openid, $orderid);
...
//                            pdo_insert("sudu8_page_pt_order",$jqrarr);
                        ...
                        foreach ($lists as $key1 => &$reb) {
                            ...
                            if($reb['coupon']!=0){
                                // 先判断优惠券有没有过期了
//                                $coupon = pdo_fetch("SELECT * FROM ".tablename('sudu8_page_coupon_user')." WHERE id = :id and uniacid = :uniacid" , array(':id' => $reb['coupon'],':uniacid' => input('uniacid']));
...
//        pdo_update("sudu8_page_user",$data,array('openid'=>$openid,'uniacid' => input('uniacid']));
...
//                pdo_update("sudu8_page_pt_order",$pdata,array("order_id"=>$order_id));
        ...
        //模板消息
//        $applet = pdo_fetch("SELECT * FROM ".tablename('account_wxapp')." WHERE uniacid = :uniacid" , array(':uniacid' => input('uniacid']));
        $applet = Db::table('applet')->where("id", $uniacid)->field('appID,appSecret')->find();
        if ($applet) {
...
//        exit();
     ...
     if($pro["show_pro"]==1){
        ...
        $uid = $userinfo['id'];
        $m_address = [];
        $addressinfo=Db::table("ims_sudu8_page_duo_products_address")->where("id",$address)->find();
    ...
    {
        $uniacid = input("uniacid");
        $baseInfo = Db::table('ims_sudu8_page_base')->where("uniacid", $uniacid)->field("name,share_open,base_tcolor,base_color,base_color2,base_color_t,tel,longitude,latitude,copy_do,copyright,copyimg,tel_b,uniacid,tabbar_bg,tabbar_tc,tabbar,color_bar,tabbar_t,tabnum,config,tabbar_tca,color_bar,address,tabbar_new,tabnum_new,tabbar_bg1,tabbar_bg2,tabbar_bg3")->find();
    ...
    public function dopageshareguiz()
    {
        ...
        // }
        $neiw['data'] = remote($uniacid, $guiz['thumb'], 1);
    ...
    public function doPageselectShopList()
    {
        ...
        $ne = input("longitude");
        $page = input('page');
        $sql = "select id,uniacid,name,logo,tel,address,(2 * 6378.137 * ASIN(SQRT(POW(SIN(PI()*(" . $le . "-latitude)/360),2)+COS(PI()*" . $le . "/180)* COS(latitude * PI()/180)*POW(SIN(PI()*(" . $ne . "-longitude)/360),2)))) as distance FROM  ims_sudu8_page_shops_shop WHERE uniacid = {$uniacid} and flag =1 and status=1";
    ...
    public function doPageis_apply()
    {
        ...
        if (!empty($shop) && $shop['status'] == '2') {
            Db::table('ims_sudu8_page_shops_shop')->where("uniacid", $uniacid)->where('openid', $openid)->delete();
    ...
    public function doPageshopApply()
    {
        ...
        $res = Db::table('ims_sudu8_page_shops_shop')->where('username', $formdata['username'])->select();
        if(count($res)>0){
            return json_encode(array('data'=>-1));
        }else{
    ...
    public function doPageshowPro_W()
    {
        ...
        if($pro['sid']){
            $pro['shopinfo'] = Db::table('ims_sudu8_page_shops_shop') ->where('uniacid', $uniacid) ->where('id', $pro['sid']) ->find();
        }
        $data = array(
    ...
    public function doPageyuzhifu_w(){
        ...
        if($userinfo['money'] < $money){
            ...
            Db::table('ims_sudu8_page_user') ->where('uniacid', $uniacid) ->where('openid', $openid) ->update(['money' => $user_money]);
        }
        $sdata = array(
            "flag" => 1
        );
        $res = Db::table('ims_sudu8_page_duo_products_order')->where('uniacid', $uniacid)->where('order_id', $order_id)->update($sdata);
        $result['data'] = $res;
        return json_encode($result);
    ...
    public function doPagezhifu_W()
    {
        ...
        $order_id = input("order_id");
        $userinfo = Db::table('ims_sudu8_page_user')->where("uniacid", $uniacid)->where("openid", $openid)->find();
        $formId = Db::table('ims_sudu8_page_duo_products_order') ->where('order_id', $order_id) ->field('formId') ->find()['formId'];
        // $data = array(
        //     'uniacid' => $uniacid,
        //     'orderid' => $order_id,
        //     'uid' => $userinfo['id'],
        ...
        $return = $weixinpay->pay();
        Db::table('ims_sudu8_page_duo_products_order') ->where('order_id', $order_id) ->update(['pay_info'=>serialize($return)]);
        $return['order_id'] = $order_id;
        $adata['data'] = $return;
        $adata['message'] = "success";
        return json_encode($adata);
    ...
    public function doPagezhifuSuccess_W()
    {
        ...
        $openid = input("openid");
        $userinfo = Db::table('ims_sudu8_page_user')->where("uniacid", $uniacid)->where("openid", $openid)->find();
        Db::table('ims_sudu8_page_user') ->where('uniacid', $uniacid) ->where('openid', $openid) ->update(['money' => 0]);
        //消费流水
        $price = Db::table('ims_sudu8_page_duo_products_order') ->where('order_id', $order_id) ->field('price') ->find()['price'];
        $data = array(
            'uniacid' => $uniacid,
            'orderid' => $order_id,
            'uid' => $userinfo['id'],
    ...
    public function dopagebeforepay()
    {
        ...
        if ($types == 'duo') {
            ...
            if($order['sid'] !=0){
                ...
                if($order['pay_info']){
                    ...
                    //将支付信息保存
                    Db::table('ims_sudu8_page_duo_products_order') ->where('uniacid', $uniacid) ->where('order_id', $order_id) ->update(['pay_info' =>serialize($weixinpay)]);
                }
                ...
                if ($price == $gpc_price && $price > $yue && round($price - $yue, 2) . '' == $payprice . '') {
                    ...
                    $jsondata = unserialize($order['jsondata']);
                    foreach ($jsondata as $key => &$value) {
                        if(isset($value['proinfo']['id'])){
                            $kc = Db::table('ims_sudu8_page_duo_products_type_value')->where("id", $value['proinfo']['id'])->find()['kc'];
                        }else{
                            $kc = $value['proinfo']['kc'];
                        }
                        if ($value['num'] > $kc) {
        ...
        if($types == "miaosha"){
            ...
            if($price == $gpc_price && $price > $yue && round($price - $yue, 2).'' == $pay_price.''){
                ...
                if($order['pay_info']){
                    ...
                    Db::table('ims_sudu8_page_order') ->where('uniacid', $uniacid) ->where('order_id', $order_id) ->update(['pay_info' =>serialize($weixinpay)]);
                }
        ...
        if($types == "yuyue"){
            ...
            if(floatval($user_money) >0 && floatval($user_money) < floatval($price)){  //余额小于实付金额并且余额大于0，修改余额为0
                // pdo_update("sudu8_page_user", array("money" => 0), array("uniacid"=>$uniacid, "openid"=>$openid));
                Db::table('ims_sudu8_page_user') ->where('uniacid', $uniacid)->where('openid', $openid) ->update(['money' => 0]);
            }
            ...
            //             // return $this->result(0, 'success', array("err"=>4, "message" => $more_type_x[$k][0]."库存不足！剩余".intval($v['shennum'])."件"));
            //             // exit;
            //             $data['data'] = array('err' => 4, 'message' => $more_type_x[$k][0]."库存不足！剩余".intval($v['shennum'])."件");
            //             return json_encode($data);
            //         }
            //     }
            // }else if($product['tableis'] == 1){
            // }
    ...
    public function doPagesavePrepayid()
    {
        ...
        if($types == 'duo'){
            ...
            Db::table('ims_sudu8_page_order')->where("uniacid", $uniacid)->where("order_id", $order_id)->update(array("prepayid" => $prepayid));
        }
        $data = array(
            'message' => 'ok',
        );
    ...
    public function doPagepayoverFxs()
    {
        ...
        if ($fxsid) {
            ...
            //获取该小程序的分销关系绑定规则
            $guiz = Db::table('ims_sudu8_page_fx_gz')->where("uniacid", $uniacid)->find();
            $fxsinfo = Db::table('ims_sudu8_page_user')->where("uniacid", $uniacid)->where("openid", $fxsid)->where("fxsstop",1)->find();
            // 1.先进行上下级关系绑定[判断是不是首次下单]
...
////                 }
//                 if ($product['fx_uni'] == 1 && $product['commission_type'] == 1) {  //商品独立分销，比例
//                     $onemoney = round($paymoney * $product['commission_one'] * $order['num'] / 100, 2);
//                     $twomoney = round($paymoney * $product['commission_two'] * $order['num'] / 100, 2);
//                     $threemoney = round($paymoney * $product['commission_three'] * $order['num'] / 100, 2);
//                 } else if ($product['fx_uni'] == 1 && $product['commission_type'] == 2) {  //商品独立分销，固定
...
//                 } else {                                                                //全局分销比例
//                     $onemoney = round($paymoney * $guiz['one_bili'] * $order['num'] / 100, 2);
//                     $twomoney = round($paymoney * $guiz['two_bili'] * $order['num'] / 100, 2);
//                     $threemoney = round($paymoney * $guiz['three_bili'] * $order['num'] / 100, 2);
//                 }
//
//             }
//
...
//                    $fxsstop1 = pdo_getcolumn('sudu8_page_user', array('uniacid' => $uniacid, 'openid' => $new_userinfo['parent_id'], 'fxs' => 2), 'fxsstop');
                     $f=Db::table("ims_sudu8_page_user")->where('uniacid',$uniacid)->where('openid',$new_userinfo['parent_id'])->where('fxs',2)->find();
                    if($f['fxsstop'] == 1){
                    $lsdata = array(
                        "uniacid" => $uniacid,
                        "openid" => $openid,
            ...
            if ($guiz['fx_cj'] == 2) {
                if ($new_userinfo['parent_id'] != '0' || $new_userinfo['p_parent_id'] != '0') {
                    $a=Db::table("ims_sudu8_page_user")->where('uniacid',$uniacid)->where('openid',$new_userinfo['parent_id'])->where('fxs',2)->find();
                    $b=Db::table("ims_sudu8_page_user")->where('uniacid',$uniacid)->where('openid',$new_userinfo['p_parent_id'])->where('fxs',2)->find();
                    if($a['fxsstop'] == 1 || $b['fxsstop'] == 1){
                        if($a['fxsstop'] == 2){
            ...
            if ($guiz['fx_cj'] == 3) {
                if ($new_userinfo['parent_id'] != '0' || $new_userinfo['p_parent_id'] != '0' || $new_userinfo['p_p_parent_id'] != '0') {
                    $c=Db::table("ims_sudu8_page_user")->where('uniacid',$uniacid)->where('openid',$new_userinfo['parent_id'])->where('fxs',2)->find();
                    $d=Db::table("ims_sudu8_page_user")->where('uniacid',$uniacid)->where('openid',$new_userinfo['p_parent_id'])->where('fxs',2)->find();
                    $e=Db::table("ims_sudu8_page_user")->where('uniacid',$uniacid)->where('openid',$new_userinfo['p_p_parent_id'])->where('fxs',2)->find();
                    if($c['fxsstop'] == 1 || $d['fxsstop'] == 1 ||$e['fxsstop'] == 1){
                        if($c['fxsstop'] == 2){
                            $new_userinfo['parent_id'] = 0;
                        }
                        if($d['fxsstop'] == 2){
        ...
        if ($is_fxs == 1) {   //支付完了还不是分销商
            $guiz = Db::table('ims_sudu8_page_fx_gz')->where("uniacid", $uniacid)->find();
            $val = $guiz['fxs_sz_val'];
            if ($guiz['fxs_sz'] == 3) {  //消费次数
                // 获取我的消费次数
                // $xf = count(Db::query("SELECT * FROM ims_sudu8_page_duo_products_order WHERE openid = '" . $openid . "' and uniacid = " . $uniacid . " and flag = 1 or flag =2"));
                $xf = Db::table('ims_sudu8_page_duo_products_order') ->where('uniacid', $uniacid) ->where('openid', $openid) ->where('flag', 'in', [1,2]) ->count();
                if ($xf + 1 >= $val) {
                    Db::table('ims_sudu8_page_user')->where('uniacid', $uniacid)->where('openid', $openid)->update(array("fxs" => 2, "fxstime" => time()));
                }
            }
            if ($guiz['fxs_sz'] == 4) {  //消费过的金额？？？？？消费的金额？？？？？分销出去消费的金额？？？？？？
                // 获取我的消费
                // $xf = Db::query("SELECT price FROM ims_sudu8_page_duo_products_order WHERE openid = '" . $openid . "' and uniacid = " . $uniacid . " and flag = 1 or flag =2");
                $xf = Db::table('ims_sudu8_page_duo_products_order') ->where('uniacid', $uniacid) ->where('openid', $openid) ->where('flag', 'in', [1,2]) ->sum('price');
                $order = Db::table('ims_sudu8_page_duo_products_order') ->where('order_id', $order_id) ->field('price') ->find();
                // 分销后重新下单的情况
                // $jjmoney = 0;
                // foreach ($xf as $key => &$res) {
                //     $jjmoney += $res['price'];
    ...
    public function dopagecreateorder()
    {
        ...
        if ($types == 'duo') {
            ...
            if (bccomp(round($payprice, 2) + floatval($yunfei), floatval($price)) == 0) {
                ...
                $m_address = [];
//                $addressinfo = pdo_fetch("SELECT * FROM ".tablename('sudu8_page_duo_products_address')." WHERE `id` = :address",array(':address' => $address));
                $addressinfo=Db::table("ims_sudu8_page_duo_products_address")->where("id",$address)->find();
                if(!empty($addressinfo)){
                    $m_address['address'] = $addressinfo['address'].' '.$addressinfo['more_address'];
                    $m_address['name'] = $addressinfo['name'];
        ...
        if($types == "yuyue"){
            ...
            if($product['tableis'] == '1' && $product['tableid'] && input('type') == 'table' && input('NowSelectStr') && input('appoint_date')){
                ...
                $selectinfo = Db::table('ims_sudu8_page_tableselect') ->where('uniacid', $uniacid) ->where('pid', $product['id']) ->where('appoint_date', $appoint_date)->where('flag', 1)->field('select_str') ->select();
                $hasselect = array();
                if($selectinfo){
                ...
                if(input('order')){
                    ...
                    $tsid = Db::table('ims_sudu8_page_tableselect') ->insertGetId($tableselect);

                    $data['tsid'] = $tsid;

                    $table = Db::table('ims_sudu8_page_table') ->where('uniacid', $uniacid) ->where('id', $product['tableid']) ->find();
                    ...
                    Db::table('ims_sudu8_page_products') ->where('uniacid', $uniacid) ->where('id', $pid) ->update(['sale_tnum' => $product['sale_tnum']]);
                }
                if($res){
                    //更新选择座位状态
                    //Db::table('ims_sudu8_page_tableselect') ->where('uniacid', $uniacid) ->where('pid', $id) ->where('appoint_date', $date) ->where('flag', 1) 
                    Db::table('ims_sudu8_page_tableselect')->where('id', $tsid) ->update(['flag' => 1]);
                    ...
                    if($dkscore != '0' && !empty($dkscore)){
                        ...
                        Db::table('ims_sudu8_page_user') ->where('uniacid', $uniacid) ->where('id', $userinfo['id']) ->update($order_userinfo);
                    }
                    
                    //处理优惠券
                    if($couponid != '0' && !empty($couponid)){
                        Db::table('ims_sudu8_page_coupon_user') ->where('uniacid', $uniacid) ->where('id', $couponid) ->update(['flag'=>1,'utime'=>time()]);
                    }
            ...
            //判断算出来的价格与实付价格是否相等
            if(bccomp(round($payprice, 2), $price) == 0){
                if($product['more_type_num']){
                    $more_type_num = unserialize($product['more_type_num']);
                }
                $newnum = explode("&quot;",$num);
                ...
                foreach ($more_type_num as $k => $v) {
                    if($numarr[$k] > $v['shennum']){
                        $more_type_x = unserialize($product['more_type_x']);
                        // return $this->result(0, 'success', array("errcode"=>3, "err" => $more_type_x[$k][0]."库存不足！", "kc"=>intval($v['shennum'])));
                        // exit;
                ...
                $data['uniacid'] = $uniacid;
                $data['order_id'] = $order_id;
                $myres = Db::table('ims_sudu8_page_order') ->insert($data);
                if($myres){
                    //处理库存与销量
                    if($product['tableis'] != 1){
                        ...
                        Db::table('ims_sudu8_page_products') ->where('uniacid', $uniacid) ->where('id', $product['id']) ->update(['sale_tnum' => $sale_tnum, 'more_type_num' => serialize($more_type_num), 'more_type'=> serialize($more_type)]);
                    }
                    // else{
                    //     //更新选择座位状态
                    //     //Db::table('ims_sudu8_page_tableselect') ->where('uniacid', $uniacid) ->where('pid', $id) ->where('appoint_date', $date) ->where('flag', 1) 
                    //     Db::table('ims_sudu8_page_tableselect')->where('id', $order['tsid']) ->update(['flag' => 1]);
                    // }
                ...
                foreach ($releaseAll as $key => &$value) {
                    ...
                    if ($pageType != 3) {
                        ...
                        foreach ($value['commentList'] as $kvs => &$vs) {
                            $vs['nickname'] = rawurldecode($vs['nickname']);
                        }
    ...
    public function doPageGetForumCon()
    {
        ...
        if($getcon['nickname']){
            $getcon['nickname'] = rawurldecode($getcon['nickname']);
        }
             ...
             //多商户商品
                if($order['payprice'] == 0){
                    if($user['money'] <$order['price']){
                        ...
                        Db::table('ims_sudu8_page_user') ->where('uniacid', $uniacid) ->where('openid', $openid) ->update(['money' => $duo_money]);
                    }
                }else{
                    Db::table('ims_sudu8_page_user') ->where('uniacid', $uniacid) ->where('openid', $openid) ->update(['money' => 0]);
                }
                ...
                //消费流水
                $price = Db::table('ims_sudu8_page_duo_products_order') ->where('order_id', $orderid) ->field('price') ->find()['price'];
                $data_r = array(
                    'uniacid' => $uniacid,
                    'orderid' => $orderid,
                    'uid' => $user['id'],
                ...
                Db::table('ims_sudu8_page_money')->insert($data_r);
                $data_e = array(
                    "flag" => 1
                );
                Db::table('ims_sudu8_page_duo_products_order')->where('uniacid', $uniacid)->where('order_id', $orderid)->update($data_e);
                $data = array(
                    'message' => '成功',
                );
                $result['data'] = $data;
...
//             }
        ...
        if($types == "miaosha"){
            ...
            $product = Db::table('ims_sudu8_page_products') ->where('uniacid', $uniacid) ->where('id', $order['pid']) ->find();
            if($product['pro_kc'] != -1 && $product['pro_kc'] < $order['num']){
                    $data = array(
                        'message' => '失败',
                    );
                    $result['data'] = $data;
        ...
        if($types == "yuyue"){
            ...
            if($product['pro_flag_ding'] == 1 && $product['tableis'] == 0){
                Db::table('ims_sudu8_page_order') ->where('uniacid', $uniacid) ->where('order_id', $orderid) ->update(['flag'=>3]);
            }else{
                Db::table('ims_sudu8_page_order') ->where('uniacid', $uniacid) ->where('order_id', $orderid) ->update(['flag'=>1]);
            }
            ...
            //更新优惠券使用情况
            // if($order['coupon'] != '0'){
            //     Db::table('ims_sudu8_page_coupon_user') ->where('uniacid', $uniacid) ->where('id', $order['coupon']) ->update(['flag'=>1,'utime'=>time()]);
            // }
            ...
            $this->sendTplMessage($uniacid, $flag2, $openid, $formId, 'duo_zf', $data8);

            //发送邮件提醒to商家
            $this->sendMailToAdmin($uniacid, $orderid, "yuyue");
    ...
    public function doPagegetConfig()
    {
        ...
        $prizesrecord = Db::table('ims_sudu8_page_staff_record') ->where('uniacid', $uniacid) ->where('operid', $user['id']) ->where('type', 'NEQ', 1) ->where('type', 'NEQ', 2) ->whereTime('createtime', 'today') ->select();
        $prizescount = 0;
    ...
    public function doPageshowPro11(){
        ...
        if(input('orderid')){
            // $pro['order_num'] = pdo_getcolumn("sudu8_page_order", array("uniacid"=>$uniacid, "order_id"=>input('orderid']), "num");
            $pro['order_num'] = Db::table('ims_sudu8_page_order') ->where('uniacid', $uniacid)->where('order_id', input('orderid')) ->field('num') ->find();
            $pro['order_num'] = intval($pro['order_num']['num']);
            // $pro['nav'] = pdo_getcolumn("sudu8_page_order", array("uniacid"=>$uniacid, "order_id"=>input('orderid']), "nav");
            $pro['nav'] = Db::table('ims_sudu8_page_order') ->where('uniacid', $uniacid) ->where('order_id', input('orderid')) ->field('nav') ->find();
            $pro['nav'] = $pro['nav']['nav'];
        }
        $data = array(
            'hits' => $pro['hits'] + 1,
    ...
    public function doPagescoreDeduction(){
        ...
        $id = input('id');
        $num = input('num');
        $proinfo = Db::table('ims_sudu8_page_products') ->where('uniacid', $uniacid) ->where('id', $id) ->field('is_more') ->find();
        if($proinfo['is_more']){
            $score_max = Db::table('ims_sudu8_page_products') ->where('uniacid', $uniacid) ->where('id', $id) ->field('score_num') ->find();
    ...
    public function doPageProTable(){
        ...
        $table = Db::table('ims_sudu8_page_table') ->where('id', $tableid) ->where('uniacid', $uniacid) ->find();
    ...
    public function doPagegetSelected(){
        ...
        $select_strs = Db::table('ims_sudu8_page_tableselect') ->where('uniacid', $uniacid) ->where('pid', $id) ->where('appoint_date', $date) ->where('flag', 1) ->select();
        $select_str = '';
        if($select_strs){
    ...
    public function doPagelvtuikuan(){
        ...
        if($order_id && $order_id != 'undefined'){
            $beforedays = Db::table('ims_sudu8_page_products') ->where('uniacid', $uniacid) ->where('id', $pid) ->field('pro_flag_data_name') ->find();
            ...
            if($beforedays > 0 && ($appoint_date - 3600 * 24 * $beforedays < time())){
                ...
                Db::table('ims_sudu8_page_order') ->where('uniacid', $uniacid) ->where('order_id',$order_id) ->update(['flag' => 6]);
            }
        }
        $data['data'] = array("flag" => 0, "message" => "申请成功！");
        return json_encode($data);
    ...
    public function doPageapplyModifyAppointInfo(){
        ...
        $data = serialize($data);
        Db::table('ims_sudu8_page_order') ->where('uniacid', $uniacid) ->where('order_id', $order_id) ->update(['modify_info' => $data]);
        $data = $appoint_date;
        return json_encode($data);
    ...
    public function doPagegetStaffs(){
        ...
        $info = Db::table('ims_sudu8_page_staff') ->where('uniacid', $uniacid) ->order('sort desc') ->select();
    ...
    public function doPagegetStaffset(){
        ...
        $staffset = Db::table('ims_sudu8_page_staffset') ->where('uniacid', $uniacid) ->find();
...
//                tabbar_url
                if(is_numeric($baseInfo['tabbar'][$i]['tabbar_linktype'])){
                    $cate_type = Db::table('ims_sudu8_page_cate') ->where('uniacid', $uniacid) ->where('id', $baseInfo['tabbar'][$i]['tabbar_linktype']) ->field('id, type, list_type') ->find();
                    if( $cate_type['type'] == "page"){
                        $baseInfo['tabbar'][$i]['type']= 'page';
                    }
                    if( $cate_type['type'] == "coupon"){
...
//
    ...
    public function dopagegetStaffInfo(){
        ...
        $oldinfo = Db::table('ims_sudu8_page_staff') ->where('uniacid', $uniacid) ->where('id', $id) ->find();
        ...
        //记录访问者信息
        if(!$oldvisitor){
            $oldvisitor = array($userinfo['id']);
        }else{
            if(!in_array($userinfo['id'], $oldvisitor)){
                if(count($oldvisitor) > 7){
                    unset($oldvisitor[7]);
                    array_unshift($oldvisitor,$userinfo['id']);
                }else{
                    array_unshift($oldvisitor,$userinfo['id']);
                }
            }
        ...
        Db::table('ims_sudu8_page_staff') ->where('uniacid', $uniacid) ->where('id', $id) ->update($data1);

        $info = Db::table('ims_sudu8_page_staff') ->where('uniacid', $uniacid) ->where('id', $id) ->find();
        ...
        if($info){
            ...
            foreach ($info['visitor'] as $key => $value) {
                $pic = Db::table('ims_sudu8_page_user') ->where('uniacid', $uniacid) ->where('uniacid', $uniacid) ->where('id', $value) ->field('avatar') ->find();
                $visitors[] = array(
                    'id' => $value,
                    'pic' => $pic['avatar'],
                );
        ...
        //查询是否已点赞
        $haszan = Db::table('ims_sudu8_page_staff_zans') ->where('uniacid', $uniacid) ->where('uid', $userinfo['id']) ->where('sid', $id) ->find();
        if($haszan){
            if($haszan['zans'] == 0){
                $data['haszan'] = 0;
            }else{
    ...
    public function doPageaddVisit(){
        ...
        // //增加访问量
        Db::execute("UPDATE ims_sudu8_page_staff set visit = visit + 1 where uniacid = ".$uniacid." and id = ".$id);
    ...
    public function doPagestaffzan(){
        ...
        if($iszan == 0){ //点赞
            Db::execute("UPDATE ims_sudu8_page_staff set zan = zan + 1 where uniacid = ".$uniacid." and id = ".$id);

            $staffinfo = Db::table('ims_sudu8_page_staff') ->where('uniacid', $uniacid) ->where('id', $id)->field('zan') ->find();
            $data = array(
                'zan' => $staffinfo['zan'],
                'result' => 1
            );

            $record  = Db::table('ims_sudu8_page_staff_zans') ->where('uniacid', $uniacid) ->where('uid', $user['id']) ->where('sid', $id) ->find();
            if($record){
                $zan1 = array(
                    'zans' => 1,
                    'createtime' => time(),
                );
                Db::table('ims_sudu8_page_staff_zans') ->where('uniacid', $uniacid) ->where('uid', $record['uid']) ->where('sid', $id) ->update($zan1);

            }else{
                Db::table('ims_sudu8_page_staff_zans') ->insert($staff_zan);
            }
            $result1['data'] = $data;
            ...
            Db::table('ims_sudu8_page_staff_zans') ->where('uniacid', $uniacid) ->where('uid', $user['id']) ->where('sid', $id) ->update($zan);
            $data1 = array(
                'zan' => $staffinfo['zan'],
                'result' => 0
            );
    ...
    public function doPagesavecard(){
        $uniacid = input('uniacid');
        $id = input('id');  //员工id
        $openid = input('openid');  //保存者openid

        //查找当天是否已有对改员工的保存操作
        $c = Db::table('ims_sudu8_page_staff_record') ->where('uniacid', $uniacid) ->where('staffid', $id) ->where('type', 'save')->whereTime('createtime', 'today') ->count();
        if($c > 0){
            $data['data'] = 10;
            return json_encode($data);
        }else{
            //查找出操作人id
            $oper = Db::table('ims_sudu8_page_user') ->where('uniacid', $uniacid) ->where('openid', $openid)->find();

            //查找出是否已操作保存过改员工
            $morecount = Db::table('ims_sudu8_page_staff_record') ->where('uniacid', $uniacid) ->where('operid', $oper['id']) ->where('staffid', $id) ->where('type', 2) ->where('createtime', 'today')->count();
            if($morecount > 0){
                $data['data'] = 10;
                return json_encode($data);
            }
            ...
            //根据小程序id 获取员工设置参数
            $staffset = Db::table('ims_sudu8_page_staffset') ->where('uniacid', $uniacid) ->field('is_save, save_score, save_scount, save_prize, save_pcount') ->find();
            ...
            if($staffset['is_save'] == 1 ){
                ...
                $savecount = Db::table('ims_sudu8_page_staff_record') ->where('uniacid', $uniacid) ->where('operid', $oper['id']) ->where('type', 'save') ->whereTime('createtime', 'today') ->count();

                if($staffset['save_scount'] == $staffset['save_pcount']){    //两个上限相等
                    if($savecount < $staffset['save_scount']){
                        Db::table('ims_sudu8_page_staff_record') ->insert($data);
                        Db::table('ims_sudu8_page_user') ->where('id', $oper['id']) ->update($score);
                        Db::table('ims_sudu8_page_score') ->insert($xfscore);
                        $data['data'] = 0;
                        return json_encode($data);
                    }else{
                        if($morecount < 1){
                            Db::table('ims_sudu8_page_staff_record') ->insert($moredate);
                        }
                        $data['data'] = 1;
                        return json_encode($data);
                    }
    ...
    public function doPagesharecardSuccess(){
        $uniacid = input('uniacid');
        $id = input('id');  //员工id
        $shareopenid = input('shareopenid');  //分享者openid
        $openid = input('openid');
        //查找出操作人id
        $oper = Db::table('ims_sudu8_page_user') ->where('uniacid', $uniacid) ->where('openid', $shareopenid) ->find();

        //查找出点击人id
        $read = Db::table('ims_sudu8_page_user') ->where('uniacid', $uniacid) ->where('openid', $openid) ->find();
        $c = Db::table('ims_sudu8_page_staff_record') ->where('uniacid', $uniacid) ->where('readid', $read['id']) ->where('staffid', $id) ->where('type', 'share')->whereTime('createtime', 'today') ->count();
        ...
        if($c > 0){
            ...
            $morecount = Db::table('ims_sudu8_page_staff_record') ->where('uniacid', $uniacid) ->where('operid', $oper['id']) ->where('readid', $read['id']) ->where('staffid', $id) ->where('type', 1) ->whereTime('createtime', 'today') ->count();

            if($morecount <1){
                Db::execute("UPDATE ims_sudu8_page_staff set forward = forward + 1 where uniacid = ".$uniacid." and id = ".$id);
            }
            ...
            //根据小程序id 获取员工设置参数
            $staffset = Db::table('ims_sudu8_page_staffset') ->where('uniacid', $uniacid) ->field('is_share, share_score, share_scount, share_prize, share_pcount') ->find();
            ...
            if($staffset['is_share'] == 1){
                ...
                //判断保存操作的次数, 超过设定的次数返回提示
                $savecount = Db::table('ims_sudu8_page_staff_record') ->where('uniacid', $uniacid) ->where('operid', $oper['id']) ->where('type', 'share') ->whereTime('createtime', 'today') ->count();
    ...
    public function doPagetoPrizes(){
        ...
        $now = time();
        //查找出当前表中的活动
        $activity = Db::table('ims_sudu8_page_lottery_activity') ->where('uniacid', $uniacid) ->where('status', 1) ->where('begin', 'ELT', $now) ->where('end', 'GT', $now) ->order('id desc') ->limit(1) ->find();
        if($activity){
    ...
    public function doPagestaffDetail(){
        ...
        $staff = Db::table('ims_sudu8_page_staff') ->where('uniacid', $uniacid) ->where('id', $id) ->find();
    ...
    public function dopagesharecard(){
        ...
        $userinfo = Db::table('ims_sudu8_page_staff') ->where('uniacid', $uniacid) ->where('id', $id) ->find();
        ...
        $sharepath = 'sudu8_page/staff_card/staff_card?id='.$id."&shareopenid=".$shareopenid;
        $data = array(
            "path" => $sharepath,
            "width" => '80'
        );
        ...
        Db::table('ims_sudu8_page_staff') ->where('uniacid', $uniacid) ->where('id', $id) ->update(['qrcode' => $imgpath]);
        // }
        $results['data'] = $imgpath;
        return json_encode($results);
    }
    ...
    public function doPageGetappindex(){
        $uniacid = input('uniacid');
        $type = input('type');
        $classification=Db::table('ims_sudu8_page_auction_column')->where("uniacid", $uniacid)->select();
        ...
        foreach ($classification as $key => $v) {
            ...
            $templist=array();
            $goods=Db::query("SELECT * FROM ims_sudu8_page_auction_goodslist WHERE uniacid = {$uniacid} and cid = {$v['id']} and isindex = 1 {$where}");
            for ($i=0; $i<count($goods); $i++) {

                $pnum=Db::table("ims_sudu8_page_auction_log")->where("auction_id",$goods[$i]['id'])->select();
    ...
    public function doPageAuctionlist(){
        ...
        $data=array();
        if ($stat==0) {//未开始 
            $d=Db::query("SELECT * FROM ims_sudu8_page_auction_goodslist WHERE starttime > '{$time}' and stat = 1 and uniacid = {$uniacid} and cid like {$cid} order by id desc limit {$pagey},10");
        ...
        elseif ($stat==1) {
            $d=Db::query("SELECT * FROM ims_sudu8_page_auction_goodslist WHERE starttime < '{$time}' and endtime > '{$time}' and uniacid = {$uniacid} and cid like {$cid} order by id desc limit {$pagey},10");
            for ($j=0; $j <sizeof($d) ; $j++) {
            ...
            $d=Db::query("SELECT * FROM ims_sudu8_page_auction_goodslist WHERE endtime < '{$time}' and stat = 2 and uniacid = {$uniacid} and cid like {$cid} order by id desc limit {$pagey},10");
            if (sizeof($d)<1) {
                $errno="nothing";
            }
        }
        elseif ($stat==3) {
            $d=Db::query("SELECT * FROM ims_sudu8_page_auction_goodslist WHERE stat = 3 and uniacid = {$uniacid} and cid like {$cid} order by id desc limit {$pagey},10");
            if (sizeof($d)<1) {
                $errno="nothing";
            }
        }
        for ($j=0; $j <sizeof($d) ; $j++) {
            $dnum=Db::table('ims_sudu8_page_auction_log')->where("uniacid", $uniacid)->where('auction_id', $d[$j]['id'])->count();
            $d[$j]['pnum']=$dnum;
    ...
    public function doPageAuctionthisok(){
        ...
        $ccc=Db::table('ims_sudu8_page_auction_goodslist')->where("id", $goodsid)->find();
        if ($ccc['stat']<2) {
            $d= Db::table('ims_sudu8_page_auction_goodslist')->where("id", $goodsid)->find();
            $stat=0;//最终的物品状态
            $user=$d['max_user'];
            if ($d['max_user']=='') {
                $stat=3;
            ...
            Db::table('ims_sudu8_page_auction_goodslist')->where("uniacid", $uniacid)->where("id", $goodsid)->update(array('stat'=>$stat));
            if ($stat==2) {
                ...
                Db::table('ims_sudu8_page_auction_order')->insert($xfmoney);
            }
        }
        $results = 1;
        return json_encode($results);
    ...
    public function doPageAuctionpage(){
        ...
        $openid = input('openid');
        $message="开始执行";
        $d=Db::table('ims_sudu8_page_auction_goodslist')->where('id', $id)->select();
        ...
        for ($i=0; $i <sizeof($d); $i++) {
            ...
            $username=Db::table('ims_sudu8_page_user')->where('openid', $d[$i]['max_user'])->where('uniacid', $uniacid)->find();
            ...
            //取消原来浏览量获取，直接增加在商品表中
            //$dnum=pdo_fetchall("select count(*) as 'num' from ims_sudu8_page_auction_log where auction_id = :gid AND uniacid= :uid ",array(':uid'=>$_W['uniacid'],':gid'=>$d[$i]['id']));
            //$num=end($dnum);
            //$d[$i]['num']=$num['num'];

            //增加拍卖量
            $dnum=Db::table('ims_sudu8_page_auction_log')->where('auction_id', $d[$i]['id'])->where('uniacid', $uniacid)->count();
            $d[$i]['pnum']=$dnum;
        ...
        $data=array('pdata'=>$d);
        $d= Db::table('ims_sudu8_page_auction_remind')->where('auction_id', $id)->where('user_id', $openid)->where('uniacid', $uniacid)->find();
        ...
        $data['re']=$re;
        Db::table('ims_sudu8_page_auction_goodslist')->where('id', $id)->where('uniacid', $uniacid)->update(array('flow'=>$newflow));
        $results['data'] = $data;
        return json_encode($results);
    }
    ...
    public function doPageAuctionorderpay(){//保证金支付支付
        ...
        $d=Db::table('ims_sudu8_page_auction_deposit')->where('user_id', $openid)->where('auction_id', $goodsid)->where('stat', '<>',0)->select();
        ...
        if(count($d)<1){
            if ($cc==null) {
                ...
                elseif ($deposit==0) {
                    ...
                    $ddd=Db::table('ims_sudu8_page_auction_goodslist')->where('id', $goodsid)->find();
                    if ($payprice<=$ddd['max_cost']) {
                        ...
                        Db::table('ims_sudu8_page_auction_goodslist')->where('id', $goodsid)->where('uniacid', $uniacid)->update(array('max_cost'=>$payprice,'max_user'=>$openid));
                        $data['errno'] = 0;
                        $data['message'] = 'nopay';
                        $data['data']['errno'] = 0;
                    }
            ...
            elseif ($cc=='readytopay') {//付款前的准备
                ...
                if ($money>$deposit) {
                    Db::table('ims_sudu8_page_user')->where('uniacid', $uniacid)->where('openid', $openid)->update(array('money'=>$money-$deposit));
                    $this->setdeposit($uniacid,$openid,2,$deposit,0,$goodsid,$out_trade_no,$out_refund_no,'','');
                    $data['errno'] = 0;
                    $data['message'] = 'ok';
                    $data['data']['errno'] = 0;
                    //余额支付发送模板消息
                    $d=Db::table('ims_sudu8_page_auction_goodslist')->where('id', $goodsid)->field('name')->find();
            ...
            if ($payprice<=$ddd['max_cost']) {
                ...
                Db::table('ims_sudu8_page_auction_goodslist')->where('uniacid', $uniacid)->where('id', $goodsid)->update(array('max_cost'=>$payprice,'max_user'=>$openid));
                $data['errno'] = 0;
                $data['message'] = 'nopay';
                $data['data']['errno'] = 0;
                return json_encode($data);
    ...
    public function setofferlog($uniacid,$user,$goods,$price){
        ...
        Db::table('ims_sudu8_page_auction_log')->insert($data);
    }
    ...
    public function setdeposit($uniacid,$openid,$stat,$deposit,$deposit_wx,$goodsid,$inorder,$outorder,$form_id,$payid){
        ...
        Db::table('ims_sudu8_page_auction_deposit')->insert($data);
    }

    public function doPageGetstatbyid(){
        $uniacid = input('uniacid');
        $id = input('id');
        $d=Db::table('ims_sudu8_page_auction_goodslist')->where('id', $id)->find();
        $st=$d['starttime'];
        $et=$d['endtime'];
        $st=strtotime($st);
        $et=strtotime($et);
    ...
    public function doPageAuctionlog(){
        ...
        if ($openid!=null) {
            $d=Db::table('ims_sudu8_page_auction_log')->where('auction_id', $goodsid)->where('uniacid', $uniacid)->where('user_id', $openid)->order('id desc')->limit(10)->select();
        }else {
            $d=Db::table('ims_sudu8_page_auction_log')->where('auction_id', $goodsid)->where('uniacid', $uniacid)->order('id desc')->limit(10)->select();
        }
        for ($i=0; $i <sizeof($d) ; $i++) {
            $dd=$d[$i];
            $username = Db::table('ims_sudu8_page_user')->where('openid', $dd['user_id'])->where('uniacid', $uniacid)->limit(10)->select();
    ...
    public function doPagePaydesok(){
        ...
        $money = input('money');
        if($money > 0){   //更新用户余额
            Db::table('ims_sudu8_page_user') ->where('uniacid', $uniacid) ->where('openid', $openid) ->update(array('money' => 0));
        }
        Db::table('ims_sudu8_page_auction_deposit')->where('user_id', $openid)->where('auction_id', $gid)->where('uniacid', $uniacid)->limit(10)->update(array('stat'=>2));

        //模板消息发送
        $d=Db::table('ims_sudu8_page_auction_goodslist')->where('id', $gid)->find();
        $data=array('auctionname'=>$d['name'],
                    'msg'=>"您缴纳拍卖保证金成功");
        $formId=Db::table('ims_sudu8_page_auction_deposit')->where('uniacid', $uniacid)->where('auction_id', $gid)->where('user_id', $openid)->find();
    ...
    public function doPagePaydesbad(){
        $uniacid = input('uniacid');
        Db::table('ims_sudu8_page_auction_deposit')->where('stat', 0)->delete();
        $results = 1;
        return json_encode($results);
    }
    ...
    public function doPageGuagua(){
        ...
        $d=Db::table('ims_sudu8_page_auction_remind')->where('uniacid', $uniacid)->where('auction_id', $gid)->where('user_id', $openid)->find();
        $message='bad';
        if ($cc=='set') {
            if ($d==false) {
                $data = array('uniacid'=>$uniacid,'user_id'=>$openid,'auction_id'=>$gid,'formid'=>$formid);
                Db::table('ims_sudu8_page_auction_remind')->insert($data);
                $message='ok';
            }
        }
        elseif ($cc=='del') {
            Db::table('ims_sudu8_page_auction_remind')->where('uniacid', $uniacid)->where('auction_id', $gid)->where('user_id', $openid)->delete();
            $message='ok';
        }
    ...
    public function doPageTopaygoods(){
        ...
        $d=Db::table('ims_sudu8_page_user')->where('uniacid', $uniacid)->where('openid', $user)->find();
        $money=$d['money'];//用户的钱
        $indis=Db::table('ims_sudu8_page_auction_deposit')->where('uniacid', $uniacid)->where('user_id', $user)->where('auction_id', $gid)->find();
        $indis=$indis['deposit']+$indis['deposit_wx'];
        if($cc==null){//判断支付类型 混合以及余额
            if ($money>($price-$indis)) {//余额足够支付
                Db::table('ims_sudu8_page_auction_order')->where('uniacid', $uniacid)->where('user_id', $user)->where('auction_id', $gid)->update(array('cost_wx'=>0,'cost_purse'=>($price-$indis),'userother'=>$other,'phone'=>$_GPC['phone']));
            ...
            //  pdo_update("sudu8_page_user", array('money'=>0), array("uniacid"=>$uniacid, "openid"=>$user));
                ...
                Db::table('ims_sudu8_page_auction_order')->where('uniacid', $uniacid)->where('user_id', $user)->where('auction_id', $gid)->update(array('order_no'=>$out_trade_no,'phone'=>$_GPC['phone'],'formid'=>$formid,'cost_wx'=>$price,'cost_purse'=>$money,'userother'=>$other));
        ...
        elseif ($cc=="okpurse") {//余额支付完成
            Db::table('ims_sudu8_page_user')->where('uniacid', $uniacid)->where('openid', $user)->update(array('money'=>$money-$price-$indisindis));
            $data=array('stat'=>1,
                        'update_at'=>date("Y-m-d H:i:s"),
            ...
            Db::table('ims_sudu8_page_auction_order')->where('uniacid', $uniacid)->where('user_id', $user)->where('auction_id', $gid)->update($data);
        }
        elseif ($cc=="okwx") {//微信支付完成
            Db::table('ims_sudu8_page_user')->where('uniacid', $uniacid)->where('openid', $user)->update(array('money'=>0));
            $data=array('stat'=>1,
                        'update_at'=>date("Y-m-d H:i:s"),
                        'paid_at'=>date("Y-m-d H:i:s"),
                        'address'=>$_GPC['address'],
            ...
            $d=Db::table('ims_sudu8_page_auction_goodslist')->where('id', $gid)->find();
            $dd=Db::table('ims_sudu8_page_auction_order')->where('uniacid', $uniacid)->where('auction_id', $gid)->where('user_id', $user)->find();
    ...
    public function doPageGetorderbyid(){
        ...
        $d=Db::table('ims_sudu8_page_auction_goodslist')->where('uniacid', $uniacid)->where('id', $id)->find();
        $d['img']=remote($uniacid, $d['img'], 1);
        $d2=Db::table('ims_sudu8_page_auction_order')->where('uniacid', $uniacid)->where('user_id', $user)->where('auction_id', $id)->find();
        $d3=Db::table('ims_sudu8_page_user')->where('uniacid', $uniacid)->where('openid', $user)->find();
    ...
    public function doPageAuctionjoinlog(){
        ...
        if ($id==2) {//竞拍中
            $d=Db::table('ims_sudu8_page_auction_log')->alias('a')->join('ims_sudu8_page_auction_goodslist b', 'a.auction_id = b.id')->where('a.user_id', $openid)->where('b.stat', 1)->where('a.uniacid', $uniacid)->order("a.auction_id desc")->field("b.*")->select();
            for ($i=0; $i <sizeof($d) ; $i++) {
                $dd=$d[$i];
                $mycost=Db::table('ims_sudu8_page_auction_log')->where('user_id', $openid)->where('uniacid', $uniacid)->where('auction_id', $dd['id'])->find();
                $mycost=$mycost['cost'];
                $d[$i]['mycost']=$mycost;
            }
        }
        elseif ($id==3) {//已拍中
            $d=Db::table('ims_sudu8_page_auction_order')->alias('a')->join('ims_sudu8_page_auction_goodslist b', 'a.auction_id = b.id')->where('a.user_id', $openid)->where('a.stat', 0)->where('a.uniacid', $uniacid)->field("a.created_at,b.*")->select();
        ...
        elseif($id==4){//未拍中
            $d=Db::table('ims_sudu8_page_auction_deposit')->alias('a')->join('ims_sudu8_page_auction_goodslist b', 'a.auction_id = b.id')->where('a.user_id', $openid)->where('b.max_user','<>', $openid)->where('b.stat', 2)->where('a.uniacid', $uniacid)->select();

            for ($i=0; $i <sizeof($d) ; $i++) {
                $dd=Db::table('ims_sudu8_page_auction_deposit')->where('uniacid', $uniacid)->where("user_id", $openid)->where('action_id', $d[$i]['action_id'])->find();
                $state=$dd['stat'];
                if ($state==1) {
                    $state="已退款";
                }else {
        ...
        elseif ($id==6) {//待发货
            $d=Db::table('ims_sudu8_page_auction_order')->alias('a')->join('ims_sudu8_page_auction_goodslist b', 'a.auction_id = b.id')->where('a.user_id', $openid)->where('a.stat', 1)->where('a.uniacid', $uniacid)->field('b.*')->select();
        }
        elseif($id==7){//已发货
            $d=Db::table('ims_sudu8_page_auction_order')->alias('a')->join('ims_sudu8_page_auction_goodslist b', 'a.auction_id = b.id')->where('a.user_id', $openid)->where('a.stat', 2)->where('a.uniacid', $uniacid)->field('a.cost,a.id as orderid,b.*')->select();
        }
        elseif($id==9){//已完成
            $d=Db::table('ims_sudu8_page_auction_order')->alias('a')->join('ims_sudu8_page_auction_goodslist b', 'a.auction_id = b.id')->where('a.user_id', $openid)->where('a.stat', 3)->where('a.uniacid', $uniacid)->field('a.cost,b.*')->select();
        }
        for ($i=0; $i <sizeof($d) ; $i++) {
    ...
    public function dopageGetWuliu(){
        ...
        $set = Db::table('ims_sudu8_page_duo_products_yunfei') ->where('uniacid', $uniacid) ->find();
        if($set['api_type'] == 3){
            if($set['appcode']){
                $kd_code = array(
                    '顺丰' => 'SFEXPRESS',
    ...
    public function dopageDataindex(){
        ...
        $now = time();
        //获取今日订单总金额
        $dataindex = Db::table('ims_sudu8_page_shops_shop') ->where('uniacid', $uniacid) ->where('openid', $openid) ->field('id, name, logo, status') ->find();
        if($dataindex){
        ...
        $sid = $dataindex['id'];
        $todayOrderMoney = Db::table('ims_sudu8_page_duo_products_order') ->where('uniacid', $uniacid) ->where('sid', $sid) ->whereTime('creattime', 'today') ->where('flag', 'in', [1,2,4]) ->sum('price');
        $allOrderNum = Db::table('ims_sudu8_page_duo_products_order') ->where('uniacid', $uniacid) ->where('sid', $sid) ->where('flag', 'in', [1,2,4]) ->count();
        $allProNum = Db::table('ims_sudu8_page_shops_goods') ->where('uniacid', $uniacid) ->where('sid', $sid) ->where('status', 1) ->count();
        $dataindex['allViewNum'] =Db::table('ims_sudu8_page_shops_goods') ->where('uniacid', $uniacid) ->where('sid', $sid) ->sum('pageview');
    ...
    public function dopageProlist(){
        ...
        $status = input('status');
        $openid = input('openid');
        $shop = Db::table('ims_sudu8_page_shops_shop') ->where('uniacid', $uniacid) ->where('openid', $openid) ->field('id') ->find();
        $sid = $shop['id'];
        $products = Db::table('ims_sudu8_page_shops_goods') ->where('uniacid', $uniacid) ->where('sid', $sid) ->where('status', $status) ->field('id,sid,uniacid,title,createtime,flag,thumb,storage,vsales,rsales,sellprice,num,status') ->select();
        foreach ($products as $key => &$res) {
          $res['createtime'] = date("Y-m-d h:m:s",$res['createtime']);
    ...
    public function doPageaddPro(){
        ...
        $id = input('id');
        if(!empty($id)){
            $res = Db::table('ims_sudu8_page_shops_goods') ->where('id', $id) ->where('uniacid', $uniacid) ->update($data);
        }else{
            $goods = Db::table('ims_sudu8_page_shops_set') ->where('uniacid', $uniacid) ->field('goods') ->find();
            $data['uniacid'] = $uniacid;
    ...
    public function dopageshowShopPro(){
        $uniacid = input('uniacid');
        $id = intval(input('id'));
        $pro = Db::table('ims_sudu8_page_shops_goods') ->where('id', $id) ->where('uniacid', $uniacid) ->find();
        $data = array(
                'pageview' => $pro['pageview'] + 1,
            );
        Db::table('ims_sudu8_page_shops_goods')->where('id', $id) ->where('uniacid', $uniacid) ->update($data);
        if($pro['images']){
            $pro['images'] = unserialize($pro['images']);
            $num = count($pro['images']);
            for($i = 0; $i < $num; $i++){
    ...
    public function dopageProdel(){
        ...
        $pid = input('pid');
        $sid = input('id');
        $res = Db::table('ims_sudu8_page_shops_goods') ->where('uniacid', $uniacid) ->where('sid', $sid) ->where('id', $pid) ->delete();
            if($res){
                $notice = 1;
            }else{
              $notice = -1;
    ...
    public function dopageshanghugoods(){
        ...
        $id = input('cid');
        $cateinfo = array();
        $shop_set = Db::table('ims_sudu8_page_shops_set') ->where('uniacid', $uniacid) ->find();

        //所有分类
        if ($id == 0) { //获取全部
            $shopInfo = Db::table('ims_sudu8_page_shops_goods') ->where('uniacid', $uniacid) ->where('hot', 1) ->order('num') ->limit($shop_set['tjnum'])  ->select();
        } else {
          //分类下商品
            $shopInfo = Db::table('ims_sudu8_page_shops_goods') ->where('uniacid', $uniacid) ->where('hot', 1) ->where('cid', $id) ->order('num') ->limit($shop_set['tjnum']) ->select();
        }
        //处理图片
    ...
    public function dopageShanghuOrderList(){
        ...
        $orderlist = Db::table('ims_sudu8_page_duo_products_order') ->where('uniacid', $uniacid) ->where('sid', $sid)->where($where) ->field('id,sid,uniacid,order_id,price,jsondata,creattime,nav,flag,m_address,hxtime,kuadi,kuaidihao') ->order('creattime desc') ->select();
    ...
    public function doPageshopInfo(){
        $uniacid = input('uniacid');
        $id = intval(input('id'));
        $item = Db::table('ims_sudu8_page_shops_shop') ->where('id', $id) ->where('uniacid', $uniacid) -> find();
        ...
        $cateList = Db::table('ims_sudu8_page_shops_cate') ->where('uniacid', $uniacid) ->select();
    ...
    public function doPageeditShop(){
        ...
        $res = Db::table('ims_sudu8_page_shops_shop') ->where('uniacid', $uniacid) ->where('id', input('id')) ->update($data);   
        if($res){
            $result = 1;
        }else{
            $result = -1;
    ...
    public function dopagegetMoney(){
        $uniacid = input('uniacid');
        $sid = input('id');
        $allMoney = Db::table('ims_sudu8_page_shops_shop') ->where('uniacid', $uniacid) ->where('id', $sid) ->field('tixian') ->find()['tixian'];
        $set = Db::table('ims_sudu8_page_shops_set') ->where('uniacid', $uniacid) ->field('minimum, tixiantype') ->find();
    ...
    public function dopagegetSzjl(){
        ...
        $sid = input('id');
        $openid = input('openid');
        $Txrecords = Db::table('ims_sudu8_page_shops_tixian') ->where('uniacid', $uniacid) ->where('sid', $sid)->field('createtime, money, flag, types') ->order('createtime desc') ->select();
        foreach ($Txrecords as $key => &$value) {
            $value['from'] = 'tx';
        }

        $Szrecords = Db::table('ims_sudu8_page_duo_products_order') ->where('uniacid', $uniacid) ->where('sid', $sid)->where('flag', 'in', [2]) ->field('price,creattime,flag') ->order('creattime desc') ->select();
    ...
    public function dopagegetTxjl(){
      $uniacid = input('uniacid');
      $sid = input('id');
      $records = Db::table('ims_sudu8_page_shops_tixian') ->where('uniacid', $uniacid) ->where('sid', $sid) ->order('createtime desc') ->select();
      $data['data'] = $records;
      return json_encode($data);
    }
    ...
    public function doPagegoTixian(){
        ...
        $beizhu = input('beizhu');
        //获取钱包余额
        $tixian = Db::table('ims_sudu8_page_shops_shop') ->where('uniacid', $uniacid) ->where('id', $sid) ->field('tixian') ->find()['tixian'];
        //获取最低提现额度
        $minimum = Db::table('ims_sudu8_page_shops_set') ->where('uniacid', $uniacid) ->field('minimum') ->find()['minimum'];
        if (!$types) {
            $message =false;
        } elseif (!$account) {
            $message =false;
        ...
        //如果没有错误，执行提现请求
            //减去用户余额
            Db::table('ims_sudu8_page_shops_shop') ->where('uniacid', $uniacid) ->where('id', $sid) ->update(['tixian' => $tixian - $money]);
            $data = array(
                  'uniacid' => $uniacid,
                  'sid' => $sid,
                  'money' => $money,
            ...
            $newid = Db::table('ims_sudu8_page_shops_tixian') ->insertGetId($data);
              //开始发邮件
            $formsConfig = Db::table('ims_sudu8_page_forms_config') ->where('uniacid', $uniacid) ->field('mail_sendto,mail_user,mail_password,mail_user_name') ->find();
            $shopname = Db::table('ims_sudu8_page_shops_shop') ->where('id', $sid) ->field('username') ->find()['username'];
    ...
    public function doPagedoSend(){
        ...
        $order = Db::table('ims_sudu8_page_duo_products_order') ->where('uniacid', $uniacid) ->where('id', $id) ->find();
        if($order['nav'] == 1){
          $data=array(
            'flag'=>4,
            'kuadi'=>$kdname,
          ...
          $result = Db::table('ims_sudu8_page_duo_products_order') ->where('uniacid', $uniacid) ->where('id', $id) ->update($data);
          if($result){
            return json_encode(array('data'=> 1));
          }
        }
...
//    商品评论开始
    ...
    public function doPageAssesspro(){
        ...
        $type = input('type');
        if($type == 'duo'){
            $order=Db::table("ims_sudu8_page_duo_products_order")->where("order_id",$order_id)->where("uniacid",$uniacid)->find();
            $order['jsondata'] = unserialize($order['jsondata']);
            foreach ($order['jsondata'] as $key => &$value) {
                $pro['pid'] =  $value['pvid'];
        ...
        }else if($type == 'miaosha' || $type == 'yuyue'){
            $pro=Db::table("ims_sudu8_page_products")->alias('a')->join('ims_sudu8_page_order b','a.id = b.pid')->where('b.order_id',$order_id)->field("a.thumb,a.title,a.id as pid")->find();
    ...
    public function doPageEvaluateSub(){
        ...
        if($add == 1){
            $data = [
             ...
             $res=Db::table("ims_sudu8_page_evaluate")->where('orderid',$orderid)->where('uniacid',$uniacid)->update($data);
        }else{
            if($type == 'duo'){
                 Db::table('ims_sudu8_page_duo_products_order')->where('order_id',$orderid)->where('uniacid',$uniacid)->update(array('assess'=>2));
            }else if($type == 'miaosha' || $type == 'yuyue'){
                Db::table('ims_sudu8_page_order')->where('order_id',$orderid)->where('uniacid',$uniacid)->update(array('assess'=>2));
            }
            $data = array(
                'uniacid' => $uniacid,
                'pid' => $pid,
               ...
               $res=Db::table("ims_sudu8_page_evaluate")->insert($data);
        }
        return json_encode($res);
    }
    ...
    public function doPageEvaluateList(){
        ...
        $where = "";
        if($flag == 1){
            $where = " and a.assess = 1";
        }else if($flag == 2){
            $where = " and a.assess = 2";
        }else if($flag == 3){
            $where = " and a.assess = 3";
        }
        ...
        if($user == 1){
            $evaluatelist=Db::query("SELECT a.*,b.nickname,b.avatar FROM ims_sudu8_page_evaluate as a LEFT JOIN ims_sudu8_page_user as b on a.openid = b.openid AND a.uniacid = b.uniacid WHERE a.uniacid = {$uniacid} AND a.openid = '{$openid}' ".$where."  order by a.id desc limit ".($pindex-1)*$psize.",{$psize}");
        }else{
         $evaluatelist=Db::query("SELECT a.*,b.nickname,b.avatar FROM ims_sudu8_page_evaluate as a LEFT JOIN ims_sudu8_page_user as b on a.openid = b.openid AND a.uniacid = b.uniacid WHERE a.uniacid = {$uniacid} AND a.pid= {$id} AND a.type='{$type}' {$where}  order by a.id desc limit ".($pindex-1)*$psize.",{$psize}");
        }
        if($evaluatelist){
            foreach ($evaluatelist as $key => &$value) {

                $is=Db::table("ims_sudu8_page_evaluate_likes")->where('uniacid',$uniacid)->where('eid',$value['id'])->where('openid',$openid)->where("likes",1)->find();
                if($is){
                    $value['addlikes'] = 1;
                }else{
                    $value['addlikes'] = 2;
    ...
    public function doPageEvaluatelikes(){
        ...
        $is=Db::table("ims_sudu8_page_evaluate_likes")->where('eid',$id)->where('openid',$openid)->where('uniacid',$uniacid)->find();
        $return = [];
        if($is){
            $is=Db::table("ims_sudu8_page_evaluate_likes")->where('eid',$id)->where('openid',$openid)->where('likes',1)->where('uniacid',$uniacid)->find();
            if($is){
                $likes = 2;
                $where = "-";
                $return['likes'] = 2;
            ...
            Db::table("ims_sudu8_page_evaluate_likes")->where('eid',$id)->where('openid',$openid)->where('uniacid',$uniacid)->update(array("likes" => $likes));
        }else{
            $data = [
                'uniacid' => $uniacid,
                'eid' => $id,
            ...
            Db::table("ims_sudu8_page_evaluate_likes")->insert($data);
            $where = "+";
            $return['likes'] = 1;
        }
        $res=Db::execute("UPDATE ims_sudu8_page_evaluate set likes = likes {$where} 1 WHERE id = {$id} AND uniacid = {$uniacid}");
        if($res){
            $return['flag'] = 1;
        }else{
            $return['flag'] = 2;
    ...
    public function doPageEvaluateDetail(){
        ...
        $id = input('id');
        $openid = input('openid');
        Db::execute("UPDATE ims_sudu8_page_evaluate set hits= hits+1 WHERE id = {$id} AND uniacid = {$uniacid}");
       $info=Db::table("ims_sudu8_page_evaluate")->alias('a')->join("ims_sudu8_page_user b",'a.openid = b.openid and a.uniacid = b.uniacid')->where("a.uniacid",$uniacid)->where('a.id',$id)->field("a.*,b.nickname,b.avatar")->find();
        if($info){
            $is=Db::table("ims_sudu8_page_evaluate_likes")->where('eid',$info['id'])->where('openid',$openid)->where('uniacid',$uniacid)->where("likes",1)->find();
            if($is){
                $info['addlikes'] = 1;
            }else{
                $info['addlikes'] = 2;
    ...
    public function dopageduoshophx(){
        ...
        $data['hxtime'] = time();
        $data['flag'] = 2;
        $r = Db::table('ims_sudu8_page_duo_products_order')->where("uniacid",$appletid)->where("order_id",$order_id)->update($data);
        if($shopid != '0'){
            $money = Db::table('ims_sudu8_page_shops_shop')->where("uniacid",$appletid)->where("id",$shopid)->find()['tixian'];
            $add = Db::table('ims_sudu8_page_duo_products_order')->where("uniacid",$appletid)->where("order_id",$order_id)->find()['price'];
            $jiesuan = Db::table('ims_sudu8_page_shops_set') ->where('uniacid', $appletid) ->find();
            $jiesuan = $jiesuan['jiesuan'];
            if (floatval($jiesuan) > 0) {
                $d = floatval($jiesuan) / 100;
                $c = $add * $d;
